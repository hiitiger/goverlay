...
float4 PS(PixelInputType input, uniform bool useTexture, uniform bool useLighting, uniform bool drawFog) : SV_Target
{
    float4 lightColor = { 0.f, 0.f, 0.f, 0.f };
    float4 texColor = float4(1, 1, 1, 1);
    ...
    //
    distToCamera = length(viewDirection);
    ...
    if(drawFog)
    {
        ...
        lightColor = lerp(lightColor, fogColor, fogLerp);
    }
...
technique11 Draw
{
    ...
    {
        SetVertexShader(CompileShader(vs_5_0, TextureVShader()));
        SetPixelShader(CompileShader(ps_5_0, PS(useTexture, useLighting, drawFog)));
    }
}
