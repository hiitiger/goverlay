...
bool injectLibW(DWORD dwProcessId, PCWSTR pszLibFile)
{
...
bool inject_with_msg_hook(DWORD pid, DWORD threadId, const std::wstring& dll)
{
	...
	if (threadId == 0)
	{
		threadId = getProcessMainThread(pid);
	}

	std::wcout << "hook " << "pid: " << pid << ", thread:" << threadId;
...
bool startInject(DWORD pid, DWORD threadId, const std::wstring& dll)
{
	...
	if (pid > 0)
	{
		if (inject_with_msg_hook(pid, threadId, dll))
		{
			succeed = true;
		}
		std::wcout << L"inject_with_msg_hook, pid:" << pid << L", result:" << succeed;
	}
